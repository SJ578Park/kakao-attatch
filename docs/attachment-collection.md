...
Apple Silicon partial probe: arm64, Apple M4, macOS 26.4.1, KakaoTalk for Mac 26.4.1 build 1181에서는 DB open과 테이블 확인까지만 통과. fresh attachment URL download는 미확인.
```

HTTP 410은 “이미 만료됨”으로 취급합니다. fatal error가 아니라 정상적인 만료 결과로 기록합니다.

Apple Silicon 26.4.1 build 1181 환경에서는 현재 문서 기준으로 첨부파일 보존 동작을 지원한다고 표시하지 않습니다. 새 첨부파일을 실제로 받거나 보낸 뒤 `attachment` JSON에서 URL 후보를 추출하고 HTTP 2xx/206 다운로드가 확인되어야 지원 환경으로 승격합니다.
