...
UI 자동화: kmsg는 send/read 보조용, archive source of truth 아님
```
...
따라서 **KakaoTalk for Mac 26.1.4 외 버전에서는 지원을 주장하기 전에 재검증**해야 합니다.
...
The verified baseline is macOS 13.7.8 with KakaoTalk for Mac 26.1.4 as of 2026-06-03. The collector uses `kakaocli` to read the local SQLCipher database in read-only mode.
